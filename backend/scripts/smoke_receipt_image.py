"""Smoke-test receipt image parsing via HttpParser. Exit 2 if PARSER_* credentials missing."""
...
def _sample_request(image_base64: str) -> ParseRequest:
    return ParseRequest(
        text="",
        wallet_names=["Карта сум", "Наличный сум"],
        expense_category_names=["Такси", "Еда", "Продукты"],
...
async def _run(jpeg_path: Path) -> int:
    image_bytes = jpeg_path.read_bytes()
    image_b64 = base64.b64encode(image_bytes).decode()
    request = _sample_request(image_b64)
    ...
    print(f"receipt_status={response.receipt_status!r}")
...
def main() -> None:
    ...
    if len(sys.argv) != 2:
        print(f"usage: {Path(sys.argv[0]).name} <path-to.jpg>", file=sys.stderr)
        raise SystemExit(1)
    ...
    raise SystemExit(asyncio.run(_run(jpeg_path)))
